class Api::Docs::TasksDoc <  ActionController::Base
  ...
  def self.new_task
    <<-EOS
    # Request
    # curl --request POST -u <token>: http://api.azendoo.com/tasks -d "task[label]="Just a task"
  ...
  def self.update_task
    <<-EOS
    # Request
    # curl --request PUT -u <token>: http://api.azendoo.com/tasks/51ee451d843668087e000004 -d "task[label]=My new task label"
